...
type=daily
action=#common:run#: /usr/bin/rr-updatenotify.sh
systemd slice=
monthly week=0
can edit from ui=1
week=1111111
app name=#common:command_line#
name=RR-UpdateNotify
...
repeat hour=8
listable=1
app args={"notify_enable":false,"notify_if_error":false,"notify_mail":"","script":"/usr/bin/rr-updatenotify.sh"} 
state=enabled
...
EOF
  ...
  rm -f "/tmpRoot/usr/bin/rr-updatenotify.sh"
