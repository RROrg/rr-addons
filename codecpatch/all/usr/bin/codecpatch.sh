...
patch_common() {
  source "/etc/VERSION"
  dsm_version="${productversion:-0} ${buildnumber:-0}-${smallfixnumber:-0}"
  if [[ ! "$dsm_version" ]]; then
...
patch() {
  patch_common
  local backup_path synocodectool_hash
  backup_path="${bin_path%??????????????}/backup"
  synocodectool_hash="$(sha1sum "$bin_path" | cut -d' ' -f1)"
  if [[ "${binhash_version_list[$synocodectool_hash]+isset}" ]]; then
    ...
    if [[ -f "$backup_path/$bin_file.$backup_identifier" ]]; then
      backup_hash="$(sha1sum "$backup_path/$bin_file.$backup_identifier" | cut -d' ' -f1)"
...
rollback() {
  ...
  if [[ "${patchhash_binhash_list[$synocodectool_hash]+isset}" ]]; then
    ...
    if ((${#backup_files[@]})); then
      ...
      echo "Checking Hash.."
      local backup_hash
      backup_hash="$(sha1sum "$backup_file" | cut -d' ' -f1)"
      if [[ "${binhash_version_list[$backup_hash]+isset}" ]]; then
        cp -p "$backup_file" \
          "$bin_path"
        echo "Backup restored successfully (DSM ${binhash_version_list[$backup_hash]})"
...
while getopts "prhl" flag; do
  ...
  l)
    ...
    ;;
  *)
    echo "Incorrect option specified in command line"
    exit 2
    ;;
  esac
done
